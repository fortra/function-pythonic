...
class RequiredResources:
    ...
    def __getitem__(self, key):
        if key >= len(self._resources.items):
            return RequiredResource(self._name, fnv1.Resource())
        return RequiredResource(self._name, self._resources.items[key])

    def __bool__(self):
        return len(self._struct) > 0
...
class RequiredResource:
    def __init__(self, name, resource):
        ...
        self.connection = Connection(resource)

    @property
    def ready(self):
        if self.observed.ready == fnv1.Ready.READY_TRUE:
            return True
        if self.observed.ready == fnv1.Ready.READY_FALSE:
            return False
        return None
...
class Results:
    def __init__(self, composite):
        self._results = composite.response.results
...
class Credentials:
    ...
    def __setattr__(self, key, credentials):
        self[key] = credentials

    def __setitem__(self, key, crdentials):
        self._request.credentials[key].credentials_data = credentials
